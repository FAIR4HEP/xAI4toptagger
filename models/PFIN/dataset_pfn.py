...
def processed2tau(x,a,N=8,preprocessed=True):
    ...
    px = (x2[:,:,0]*torch.cos(x2[:,:,2])).reshape(-1,n_constits,1)
    py = (x2[:,:,0]*torch.sin(x2[:,:,2])).reshape(-1,n_constits,1)
    pz = (x2[:,:,0]*torch.sinh(x2[:,:,1])).reshape(-1,n_constits,1)
...
class PFNDataset(Dataset):
    def __init__(self, file_path, n_constits=200, preprocessed=True, frac=1.0):
        
        # Read in hdf5 files
        store = pd.HDFStore(file_path)
        df = store.select("table").sample(frac=frac)
        if n_constits > 200:
            n_constits = 200 # use at most 200 highest pt jet constituents 
        ...
        self.mask = self.mask.reshape(-1, 1, n_constits)
        self.data = torch.reshape(self.data, (-1, n_constits, 3))
        #gets rid of phi/eta subtraction from zero-vectors
        m=self.mask.reshape(-1,n_constits,1)
    ...
    def __getitem__(self, idx):
        ...
        return item, mask, label, aug_data, 0
...
if __name__ == "__main__":
    ...
    for i,(x,m,l,a,_) in enumerate(trainloader):
        print(x.shape)
        print(m.shape)
        print(l.shape)
        print(a.shape)
