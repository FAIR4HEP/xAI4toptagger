...
class AugParticleFlowNetwork(nn.Module):
    r"""Parameters
    ----------
    input_dims : int
    ...
    def __init__(self, input_dims=4, num_classes=2,
        ...
        super(AugParticleFlowNetwork, self).__init__(**kwargs)
        ...
        for i in range(len(F_sizes)):
            f_layers.append(nn.Sequential(
                nn.Linear((Phi_sizes[-1] + aug_size) if i == 0 else F_sizes[i - 1], F_sizes[i]),
                nn.ReLU())
            )
    ...
    def forward(self, features, mask, aug):
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        y = model(x, m, a)
        print(y)
    summary(model, ((1, 200, 3), (1, 1, 200), (1,7)))
